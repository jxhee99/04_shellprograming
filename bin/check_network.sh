...
if [ $? -eq 0 ] ; then
    print_good "Local Network Conneciton"
else
    print_error "Local Network Conneciton"
    echo "  (a) 물리적인 연결 확인"
    echo "  # ethtool <NIC>"
    echo "  (b) IP/NETMASK 확인"
    echo "  # ip addr"
    echo "  (c) VMware VMnet8 설정"
    echo "  VMware > Edit > Virtual Network Editot > VMnet8 > 192.168.10.0/255.255.255.0"
...
if [ $? -eq 0 ] ; then
    print_good "External Network Conneciton"
else
    print_error "External Network Conneciton"
